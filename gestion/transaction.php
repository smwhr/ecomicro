...
//	  	die("<br> PB de vérification1, !!!");
	  ...
	  $sql = "SELECT idcompte FROM eco_banque, eco_entreprise, eco_pays WHERE eco_banque.idcompte = '$idcpte1' ";
	  $sql .= "AND (eco_banque.idtitulaire = '$idjoueur' ";
	  $sql .= "OR (eco_banque.idtitulaire = eco_entreprise.identreprise AND eco_entreprise.iduser = '$idjoueur') ";
	  $sql .= "OR (eco_banque.idtitulaire = eco_pays.idpays AND eco_pays.iduser = '$idjoueur')) ;";
	  $res = @mysqli_query($conn, $sql)or die("<br> PB de vérification2, !!!");
	  $num = @mysqli_num_rows($res) or die("<br> PB de vérification2, !!!");
	}
