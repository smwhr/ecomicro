...
function Finance(){
    ...
    temp += "<tr>";
    temp += "<td colspan='13'><br>&nbsp;Evolution de la trésorerie sur les derniers mois:</td>";
    temp += "</tr>";


    for (ind01 = 0; ind01 < FINANCE_RESULTAT.length; ind01++){
        if (ind01 > 0)  
            temp += "</tr>";
        ...
        for (ind02 = 0; ind02 < 5; ind02++){
            if (FINANCE_RESULTAT[ind01][ind02] > ' '){
				if ((ind02 != 3) || (ind01 == 0))
					temp += "<td align=center width='70'>" + FINANCE_RESULTAT[ind01][ind02] + "</td>";
				else{
					if ((FINANCE_RESULTAT[ind01][ind02] > 0))
						temp += "<td align=center width='70'>+" + FINANCE_RESULTAT[ind01][ind02] + "</td>";
					else{
					
						temp += "<td align=center width='70'><FONT color='red'>" + FINANCE_RESULTAT[ind01][ind02] + "</FONT></td>";
					}
				}
			}
            else
...
//            if (LIST_CIT_IDPAYS[ind01] == IDPAYS){
                if (DET_ENTRE_IDUSER == LIST_CIT_IDUSER[ind01])
                    temp += "<OPTION selected=TRUE value='" + LIST_CIT_IDUSER[ind01] + "'>";
                else
                    temp += "<OPTION value='" + LIST_CIT_IDUSER[ind01] + "'>";
		temp += LIST_CIT_NOMPAYS[ind01] + "&nbsp;-&nbsp;&nbsp;" + LIST_CIT_NOM[ind01] + "&nbsp;&nbsp;";
//            }
        }
        temp += "</SELECT></td>";
        temp += "</tr>";
