...
<html>
<head>
<title> Message d'achat de produit </title>
<head>
<body>
...
<?php
  ...
  if ($_SESSION['perso_droituser'] != '999')
  {
		...
		else
	  {
	    ...
	    $sql .= "WHERE eco_banque.idtitulaire = '$entreA' AND eco_banque.idcompte = '$idcpte1' ";
	    $sql .= "AND eco_banque.devise = '$deviseA' ;";
	    $res = @mysqli_query($conn, $sql) or die ($sql); 
//die("<br> PB dans la requ�te de v�rification2...");
...
// die("<br> PB dans la requ�te de v�rification2...");
   ...
   $sql .= "AND eco_banque.devise = '$deviseB' ;";
    ...
    $num = @mysqli_num_rows($res) or die("<br> PB dans la requete de vérification4...");
  ...
  if ($idjoueur != $entreA)
  {
    ...
    $num = @mysqli_num_rows($res) or die("<br> L'entreprise n'existe pas ou n'est pas la votre !!!");
  ...
  {
    $sql = "SELECT nom FROM eco_user WHERE iduser = '$entreA' AND iduser = '$idjoueur';";
  ...
  $num = @mysqli_num_rows($res) or die("<br> L'entreprise 2 n'existe pas !!!");
  $produit = mysqli_fetch_array($res);
  $nomentreprise = $produit['nomentreprise'];
 ...
 $res = @mysqli_query($conn, $sql) or die("Erreur dans la requete de recherche du taux de change (mes_achatproduit) !!!");
  $num = @mysqli_num_rows($res) or die("Le taux de change n'existe pas !!!");
  $produit = mysqli_fetch_array($res);
  $taux = $produit['taux'];

  $tarif_unit = $tarif * $taux;
  $tarif_total = $tarif_unit * $nbprod;

 $libelletransaction = $nom . " propose d'acheter " . $nbprod . " " . $nomprod . " ( " . $nbunite . " " . $nomunite . " ) au tarif unitaire HT de " . $tarif_unit . " " . $deviseB . " (" . $tarif . " " . $deviseA . ") à " . $nomentreprise;
 ...
 $res = mysqli_query($conn, $sql) or die("Erreur dans la requete d'insertion du message (mes_achatproduit) !!!");
 ...
 echo "<script language=\"JavaScript\"> document.location.replace(\"../achat_produit.php\");</script>";
