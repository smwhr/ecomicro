...
<title> Message d'achat de stock </title>
...
<?php
...

  {
...

	  {
	    ...
	    $sql .= "WHERE eco_banque.idtitulaire = '$�djoueur' AND eco_banque.idcompte = '$idcpte1' ";

	    $sql .= "AND eco_banque.devise = '$deviseA' ;";

	    $res = @mysqli_query($conn, $sql)or die("<br> PB dans la requete de vérification2...");

	    $num = @mysqli_num_rows($res) or die("<br> PB dans la requete de vérification2...");
  ...
  if ($idunite != '80008')  // D�chets (PDt)

  {

    $libelletransaction = "Proposition d'achat de " . $nbunite . " " . $nomunite . " au tarif unitaire HT de " . $tarif_unit . " " . $deviseB . " (" . $tarif . " " . $deviseA . ") par " . $nom . " à " . $nomentreprise;
...

  {
    ...
    $libelletransaction = "Proposition de retraitement pour " . $nomentreprise . " de " . $nbunite . " " . $nomunite . " au tarif unitaire HT de " . $tarif_unit . " " . $deviseB . " (" . $tarif . " " . $deviseA . ") par " . $nom;
    $libelletransaction .= ". Le compte débité du total HT (" . $tarif_total . " " . $deviseB . ") est le n° : " . $idcpte2;
    $libelletransaction = addslashes($libelletransaction);
   $datatransaction = $idcpte1 . "|" . $idcpte2 . "|" . $entreA . "|" . $entreB . "|" . $nbunite . "|" . $idunite . "|" . $tarif;
 ...
 $tt += (5 * 24 * 3600);
  ...
  $res = mysqli_query($conn, $sql) or die("Erreur dans la requ�te d'insertion du message (mes_achatstock) !!!");

  // Envoi du message
  include("../include/fct_mes_newmessage.php");

  echo "<script language=\"JavaScript\"> document.location.replace(\"../achat_stock.php\");</script>";
?>
</body>
</html>