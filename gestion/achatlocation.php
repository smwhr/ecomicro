...
$fct_mes_vente_emaileco = $produit['emaileco'];
      ...
      if ($deviseA != $deviseB)
      {
        ...
        // Vérification de la devise et du solde du compte
      ...
      if ($tarif_total_taxe > 0)
      {
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête débitrice taxe (achatpa) !!!");
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête créditrice taxe (achatpa) !!!");
      }
      ...
      $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête maj compte taxe (achatpa) !!!");
      ...
      if ($deviseA == $deviseB)   // Pas de taux de change
      {
        ...
        $res = @mysqli_query($conn, $sql) or die("Erreur dans la requête débitrice (achatpa) !!!");
        ...
        $res = @mysqli_query($conn, $sql) or die("Erreur dans la requête créditrice (achatpa) !!!");
      ...
      else
      {
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête débitrice Acheteur (achatpa) !!!");
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête créditrice Banque nationale (achatpa) !!!");

        // Acheteur -> Banque 1 : solde Acheteur
        $sql = "UPDATE eco_banque SET solde = '$newsoldeA' WHERE idcompte = '$idcpte1';";
        $res = @mysqli_query($conn, $sql) or die("<br> PB de màj le solde solde Acheteur.");

        // Acheteur -> Banque 1 : solde Banque 1 dèjà fait avec Taxe
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête créditrice Vendeur (achatpa) !!!");
        ...
        $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête débitrice Banque 2 (achatpa) !!!");

        // Màj solde crédité vendeur
        $sql = "UPDATE eco_banque SET solde = '$newsoldeB' WHERE idcompte = '$idcpte2';";
        $res = @mysqli_query($conn, $sql) or die("<br> PB de màj le solde Vendeur. (achatpa)");

        // Màj solde débité banque taux
      ...
      $res = @mysqli_query($conn, $sql) or die("<br> PB de màj le message. (achatpa)");
        ...
        if ($type_prodequi == '20000')
        {
                ...
                $res = @mysqli_query($conn, $sql)or die("Erreur dans la requête immo (achatpa) !!!");
        }
