...
<html>
<head>
    <title> Message de chgt relation </title>
<head>
...
<?php
  ...
  $eco = addslashes(trim($_POST['eco']));
  $visionav = addslashes(trim($_POST['visionav']));
  $ecoav = addslashes(trim($_POST['ecoav']));
  ...
  $type = "";
  if (($visionav == '1') && ($vision == '0')) {
    $type .= "diplomatiques";
    if (($eco == '0'))
      $type .= "et économiques";
  }
  else if (($ecoav == '1') && ($eco == '0'))
     $type .= "économiques";

    $sql = "SELECT iduser,nompays FROM eco_pays WHERE idpays = '$idpaysA';";
    $res = @mysqli_query($conn, $sql)or die("<br> PB dans la requête recherche nom pays1");
    $num = @mysqli_num_rows($res) or die("<br> Le pays 1 n'éxiste pas !!!");
    $produit = mysqli_fetch_array($res);
    $nompaysA = $produit['nompays'];
    $directeur_paysA = $produit['iduser'];

    $sql = "SELECT iduser,nompays FROM eco_pays WHERE idpays = '$idpaysB';";
    $res = @mysqli_query($conn, $sql)or die("<br> PB dans la requête recherche nom pays1");
    $num = @mysqli_num_rows($res) or die("<br> Le pays 2 n'éxiste pas !!!");
    $produit = mysqli_fetch_array($res);
  ...
  $libelletransaction = "Le responsable du pays " . $nompaysA . " propose d'ouvrir les relations " . $type;
