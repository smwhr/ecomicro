...
if (!$_SESSION['perso_iduser']){
    $verification ="formulaire.php?retour=info_cpte.php";
...
<?php
    if (substr($_SESSION['perso_droituser'],1,1) >= '4')
        require("gestion/gest_info_cpte.php");
    else
        require("gestion/gest_info_user.php");
?>

<script language="javascript" type="text/javascript" >
function credcompte(){
    if ((isNaN(parseInt($("#cred").val()))) || (parseInt($("#cred").val()) <= 0 )){
        alert("Il s'agit de saisir un nombre entié strictement positif.");
        return;
    }
    else{
        $('#page-loader').show();
        document.credcompte1.action="gestion/credcompte.php";
        document.credcompte1.submit();
    }
}

function debcompte(){
    if ((isNaN(parseInt($("#deb").val()))) || (parseInt($("#deb").val()) <= 0 )){
        alert("Il s'agit de saisir un nombre entié strictement positif.");
        return;
    }
    else{
        $('#page-loader').show();
        document.debcompte1.action="gestion/debcompte.php";
        document.debcompte1.submit();
    }
}
...
<script language="Javascript" type="text/javascript" src="gestion/gest_info_cpte.js"></script>
                ...
                <tr>
                    <td valign=top>
                        ...
                        <div id="enteted" class="cadreClair textPetit"> 
                            <li>Liste les comptes bancaires des citoyens, des sociétés et provinces de votre nation (réservé au responsable économique).
                            <li>En cliquant sur un numéro de compte vous pouvez visualiser à droite le détail des mouvements.
                            <li>Apparaissent en bleu, les comptes des ciotyens, en gras les comptes des entreprises et provinces.
                        </div>
                    </td>
                </tr>
            </table>
